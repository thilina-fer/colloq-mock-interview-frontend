import { Github, Linkedin, Mail } from "lucide-react";

export default function Footer() {
  const currentYear = new Date().getFullYear();

  const footerLinks = {
    Product: ["Features", "Pricing", "Security", "Enterprise"],
    Company: ["About", "Blog", "Careers", "Press"],
    Resources: ["Docs", "API", "Support", "Community"],
    Legal: ["Privacy", "Terms", "Cookies", "Contact"],
  };

  return (
    <footer className="bg-light-surface border-t border-light-border">
      {/* Main Footer */}
      <div className="section-container py-16">
        {/* Brand & Social */}
        <div className="grid grid-cols-1 md:grid-cols-5 gap-12 mb-12 pb-12 border-b border-light-gray-200">
          {/* Brand Section */}
          <div className="md:col-span-1">
            <h3 className="text-2xl font-bold text-light-text-primary mb-2">
              ColloQ
            </h3>
            <p className="text-sm text-light-text-muted leading-relaxed">
              Master your interviews with real-world practice and expert
              feedback.
            </p>
            <div className="flex gap-3 mt-6">
              <a
                href="#"
                className="p-2 bg-light-gray-100 rounded-lg text-light-text-primary hover:bg-light-gray-200 transition-colors"
                aria-label="GitHub"
              >
                <Github size={18} />
              </a>
              <a
                href="#"
                className="p-2 bg-light-gray-100 rounded-lg text-light-text-primary hover:bg-light-gray-200 transition-colors"
                aria-label="LinkedIn"
              >
                <Linkedin size={18} />
              </a>
              <a
                href="#"
                className="p-2 bg-light-gray-100 rounded-lg text-light-text-primary hover:bg-light-gray-200 transition-colors"
                aria-label="Email"
              >
                <Mail size={18} />
              </a>
            </div>
          </div>

          {/* Links */}
          {Object.entries(footerLinks).map(([category, links]) => (
            <div key={category}>
              <h4 className="text-sm font-semibold text-light-text-primary mb-4">
                {category}
              </h4>
              <ul className="space-y-3">
                {links.map((link) => (
                  <li key={link}>
                    <a
                      href="#"
                      className="text-sm text-light-text-muted hover:text-light-text-primary transition-colors"
                    >
                      {link}
                    </a>
                  </li>
                ))}
              </ul>
            </div>
          ))}
        </div>

        {/* Bottom Bar */}
        <div className="flex flex-col sm:flex-row items-center justify-between">
          <p className="text-sm text-light-text-muted">
            © {currentYear} ColloQ. All rights reserved.
          </p>
          <div className="flex gap-4 mt-4 sm:mt-0">
            <a
              href="#"
              className="text-sm text-light-text-muted hover:text-light-text-primary transition-colors"
            >
              Privacy Policy
            </a>
            <span className="text-light-gray-300">•</span>
            <a
              href="#"
              className="text-sm text-light-text-muted hover:text-light-text-primary transition-colors"
            >
              Terms of Service
            </a>
          </div>
        </div>
      </div>
    </footer>
  );
}
